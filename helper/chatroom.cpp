...
void SetChatoomName(wchar_t* roomwxid, wchar_t* roomname)
{
	...
	DWORD dwCall1 = getWeChatWinAddr() + SETCHATROOMNAME;
	__asm
	{
		mov edx, asmRoomname;
		mov ecx, asmWxid;
...
{
	DWORD callAdrress = getWeChatWinAddr() + SETCHATROOMANNOUNCEMENT;
	CHAR bufferA[0xD0] = { 0 };
...
{
	DWORD dwCallAddr1 = getWeChatWinAddr() + ADDCHATROOMMEMBER1;
	DWORD dwCallAddr2 = getWeChatWinAddr() + ADDCHATROOMMEMBER2;
	DWORD dwCallAddr3 = getWeChatWinAddr() + ADDCHATROOMMEMBER3;
	DWORD dwCallAddr4 = getWeChatWinAddr() + ADDCHATROOMMEMBER4;

	DWORD dwParam = getWeChatWinAddr() + ADDCHATROOMPARAM1;
	DWORD dwParam2 = getWeChatWinAddr() + ADDCHATROOMPARAM2;
...
void DeleteChatroomMember(wchar_t* roomid, wchar_t* memberwxid)
{
	...
	DWORD dwCall1 = getWeChatWinAddr() + DELETECHATROOMMEMBER1;
	DWORD dwCall2 = getWeChatWinAddr() + DELETECHATROOMMEMBER2;
	DWORD dwCall3 = getWeChatWinAddr() + DELETECHATROOMMEMBER3;
...
void SendChatroomAtMsg(wchar_t* chatroomid, wchar_t* memberwxid, wchar_t* membernickname, wchar_t* msg)
{
	...
	DWORD callAddress_SendText = getWeChatWinAddr() + SENDTEXTADDR;
...
void GetChatroomUser(wchar_t* chatroomid)
{
	struct wxStr
	{
		wchar_t* pStr;
	...
	for (int i = 0; i < Len; i++)
	{
		...
		MessageBox(NULL, tempWxid, L"提示", 0);
	}
}
...
void QuitChatRoom(wchar_t* chatroomwxid)
{
	...
	DWORD dwCallAddr = getWeChatWinAddr() + QUITCHATROOM;
